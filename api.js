...
function formatDate(isoString) {
    ...
    if (diffMins < 1)
        return Lang.t('time.justnow');
    if (diffMins < 60)
        return `${diffMins} ${Lang.t('time.minutes')}`;
    if (diffHours < 24)
        return `${diffHours} ${Lang.t('time.hours')}`;
    if (diffDays < 7)
        return `${diffDays} ${Lang.t('time.days')}`;
...
async function uploadImage(base64) {
    ...
    if (!json.success)
        throw new Error(Lang.t('toast.photo.error'));
...
function attachItemCardEvents(grid, options = {}) {
    ...
    grid.querySelectorAll('.item-card-image img').forEach(img => {
        img.addEventListener('error', function () {
            ...
            if (card) {
                const cat = (_a = card.querySelector('.category-badge')) === null || _a === void 0 ? void 0 : _a.dataset.category;
...
async function createItem(newItem) {
    ...
    if (!success)
        throw new Error(Lang.t('toast.create.error'));
    return newItem;
}
/** Обновить объявление */
async function updateItem(id, updates) {
    ...
    if (index === -1)
        throw new Error(Lang.t('detail.notfound'));
    ...
    const success = await saveAll(data);
    if (!success)
        throw new Error(Lang.t('toast.update.error'));
}
...
async function deleteItem(id) {
    ...
    const success = await saveAll(data);
    if (!success)
        throw new Error(Lang.t('toast.delete.error'));
}
...
async function restoreItem(id) {
    ...
    if (!item)
        throw new Error(Lang.t('detail.notfound'));
    if (item.author !== Auth.getUser())
        throw new Error(Lang.t('create.error.permission'));
    item.status = 'active';
    delete item.deletedAt;
    const success = await saveAll(data);
    if (!success)
        throw new Error(Lang.t('toast.restore.error'));
}
...
async function toggleItemSold(id) {
    ...
    const success = await saveAll(data);
    if (!success)
        throw new Error(Lang.t('toast.status.error'));
    return item.status;
}
...
function validateItemForm(title, description) {
    if (!title || title.trim().length < 3)
        return Lang.t('validate.title.len');
    if (description && description.length > 5000)
        return Lang.t('validate.desc.len');
    return null;
}
...
async function sendMessage(itemId, itemTitle, to, text) {
    ...
    if (!ok)
        throw new Error(Lang.t('toast.send.error'));
    return msg;
}
...
function getMessages(data, itemId, otherUser) {
    ...
    if (!user || !data.messages)
        return [];
    return data.messages.filter(m => m.itemId === itemId &&
        ((m.from === user && m.to === otherUser) || (m.from === otherUser && m.to === user))).sort((a, b) => +new Date(a.createdAt) - +new Date(b.createdAt));
}
