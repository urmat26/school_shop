...
const Auth = {
    ...
    _showConfirm(title, subtitle, okText, okClass, action) {
        ...
        const onEscape = (e) => { if (e.key === 'Escape')
            cleanup(); };
    ...
    logout() {
        this._showConfirm(Lang.t('auth.logout.confirm.title'), Lang.t('auth.logout.confirm.sub'), Lang.t('auth.logout.confirm.ok'), 'btn-danger', () => {
    ...
    async register(username, password) {
        ...
        if (!ok)
            throw new Error(Lang.t('auth.register.error'));
    ...
    async login(username, password) {
        ...
        if (!user)
            throw new Error(Lang.t('auth.notfound'));
        const hash = await this.hashPassword(password);
        if (hash !== user.passwordHash)
            throw new Error(Lang.t('auth.wrongpass'));
        this.setUser(username);
        this.updateUI();
        Favorites.loadFromServer();
    },
    showModal(mode, callback) {
        ...
        loginInput.onkeydown = (e) => { if (e.key === 'Enter')
            doLogin(); clearError(); };
        loginPass.onkeydown = (e) => { if (e.key === 'Enter')
            doLogin(); clearError(); };
        // ── Register ──
        ...
        regInput.onkeydown = (e) => { if (e.key === 'Enter')
            doRegister(); clearError(); };
        regPass.onkeydown = (e) => { if (e.key === 'Enter')
            doRegister(); clearError(); };
        regConfirm.onkeydown = (e) => { if (e.key === 'Enter')
            doRegister(); clearError(); };
    },
    hideModal() {
        const modal = document.getElementById('auth-modal');
